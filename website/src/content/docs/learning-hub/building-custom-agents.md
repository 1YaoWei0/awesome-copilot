...
|----------|-------------------|
| Highest capability, most demanding tasks | Claude Opus 5 *(v1.0.75+)* |
...
> **Tip — Plan mode model**: Use `/model plan` (or `/model --plan`) to select a separate model for plan mode. This lets you use a lighter model while planning and a more capable model during execution, or vice versa. Omit the model ID to open the picker, or pass `off` to clear the plan-mode override.
