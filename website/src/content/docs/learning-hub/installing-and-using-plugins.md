...
First-party plugins (those from official GitHub Copilot marketplaces) automatically update to the latest version at session start. You don't need to run `copilot plugin update` manually for these — they stay current automatically whenever you start a new session.
