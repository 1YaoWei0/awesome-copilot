...
*(v1.0.74+)* Use `/model plan` (or `/model --plan`) to pick a **separate model for plan mode**. This lets you use a faster, lighter model during planning while reserving a more powerful model for implementation:

```
/model plan             # open the model picker scoped to plan mode
/model --plan sonnet    # set a specific model for plan mode
/model --plan off       # clear the plan-mode model override (revert to session model)
```

When you leave plan mode, the session automatically reverts to the regular session model. This is useful when you want plan proposals to be fast and low-cost but implementations to use maximum reasoning.
